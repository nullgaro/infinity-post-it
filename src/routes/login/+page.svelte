...
<template lang="pug">
  div(class="min-h-screen w-full bg-p-light-gray flex flex-col items-center justify-between")
    ...
    div(class="h-96 w-4/5 sm:w-[30rem] my-20 bg-p-gray rounded-lg flex flex-col items-center justify-center gap-5")
      h2(class="text-p-white text-3xl") Login
      form(on:submit|preventDefault="{login}" class="h-2/4 w-11/12 flex flex-col justify-evenly items-center")
        input(name="username" type="text" placeholder="Username" class="h-8 w-4/6 p-4 rounded-md bg-p-white border border-solid border-p-gray")
